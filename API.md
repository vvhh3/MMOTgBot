...
- HP обоих фиксируется **в момент accept** — «подлечиться» зельем посреди чужой дуэли нельзя.
...
Ошибки: `400` — некорректный `toPlayerId`; `404` — игрок не найден; `409` — обмен с самим собой, один из игроков мёртв (HP = 0) или уже в другой дуэли.
...
Ошибки: `400` — неизвестное действие; `404` — активная дуэль не найдена; `409` — сейчас не ваш ход.
